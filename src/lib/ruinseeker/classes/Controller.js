...
const regexValidComposition = regexValidComposite(
  regexWord,
  regexWhitespace,
  regexSpecialRune
);
const regexIdentifyFigures = regexIdentifyGroups(
  regexWord,
  regexWhitespace,
  regexSpecialRune
);
...
SVG.Controller = class extends SVG.Svg {
  ...
  generate(rawText, successCallback, failureCallback) {
    ...
    if (regexValidComposition.test(rawText)) {
      ...
      if (noTranslation.length > 0) {
        failureCallback(
          "The following cannot be tranlated: " + noTranslation.join(", ")
        );
        console.error(
          "The current string fails to pass the generation requirements."
        );
        return this;
      }
    } else {
  ...
  pushSpecialRune(groupText) {
    const specialRuneName = groupText
      .replace(/^\{+/, "")
      .replace(/\}+$/, "")
      .toLowerCase(); // Strip '{' and '}' characters
    ...
    if (svgText) {
      let newSpecialRune = this.specialrune(
        this.props,
        specialRuneName,
        svgText
      );
    ...
    for (const currentFigure of this.allFiguresList) {
      if (currentFigure.isRuneWord() || currentFigure.isSpecialRune()) {
        ...
        if (currentFigure.relevantNewlines > 0) {
          ...
          currentFigure.x(0);
          currentFigure.y(
            currentFigure.relevantNewlines * 3 * this.props.segmentLength
          );

          // Set next root
          rootX =
            currentFigure.relevantSpaces *
            this.props.fullWidth *
            (RUNE_WIDTH_PERCENT_SPACE / 100); //TODO: Fix size and constants
          rootY +=
            currentFigure.relevantNewlines *
            this.props.fullHeight *
            (1 + RUNE_HEIGHT_PERCENT_NEWLINE / 100);
  ...
  updateScaleProps(segmentLength, lineWidth) {
    ...
    this.props.fullHeight = 3.5 * segmentLength + lineWidth;
