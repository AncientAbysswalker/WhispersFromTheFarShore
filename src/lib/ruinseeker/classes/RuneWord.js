...
import phoneMap from "../../../ipa/ipa_phoneme_to_bytecode.js";
...
SVG.RuneWord = class extends SVG.Figure {
  ...
  init(props, wordString) {
    ...
    let searchTheDictionary =
      this.props.ipaDict[this.wordString] ||
      this.props.customIpaDict[this.wordString];
    if (!searchTheDictionary) {
      console.error(
        "Cannot create rune word! Provided word is not in the dictionary."
      );
      return this;
    }

    // Get the list of different possible phone combinations and select the first as the one that we want by default
    this.possiblePhones = searchTheDictionary.map((phoneOption) =>
      phoneOption.split(" ")
    ); // TODO Perhaps change with new parser! No need to split on " "?
  ...
  generateRunesFromCurrentPhones() {
    ...
    while (i < this.currentPhones.length) {
      ...
      if (
        i === this.currentPhones.length - 1 ||
        !(
          phoneMap[this.currentPhones[i]].isVowel ^
          phoneMap[this.currentPhones[i + 1]].isVowel
        )
      ) {
  ...
  setDatums() {
    ...
    this.leftDatum = 0;
    this.rightDatum =
      this.props.innerWidth * this.runes.length + this.props.lineWidth;
  }
